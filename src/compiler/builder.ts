...
namespace ts {
     ...
     *  This is because even though js emit doesn't change, dts emit / type used can change resulting in need for dts emit and js change
     */
    ...
    function handleDtsMayChangeOf(
        ...
        if (!state.changedFilesSet.has(path)) {
            ...
            if (sourceFile) {
                // Even though the js emit doesn't change and we are already handling dts emit and semantic diagnostics
                // we need to update the signature to reflect correctness of the signature(which is output d.ts emit) of this file
                // This ensures that we don't later during incremental builds considering wrong signature.
    ...
    function handleDtsMayChangeOfReferencingExportOfAffectedFile(
        ...
        // Since isolated modules don't change js files, files affected by change in signature is itself
    ...
    function handleDtsMayChangeOfFileAndExportsOfFile(
        ...
        state.referencedMap!.getKeys(filePath)?.forEach(referencingFilePath =>
            ...
            handleDtsMayChangeOf( // Don't add to seen since this is not yet done with the export removal
                state,
                referencingFilePath,
                cancellationToken,
                computeHash,
    ...
    export interface ProgramMultiFileEmitBuildInfo {
        ...
        // Because this is only output file in the program, we don't need fileId to deduplicate name
        ...
        const fileInfos = arrayFrom(state.fileInfos.entries(), ([key, value]): ProgramBuildInfoFileInfo => {
            ...
            return value.version === actualSignature ?
                value.affectsGlobalScope || value.impliedFormat ?
                    // If file version is same as signature, don't serialize signature
    ...
    export function createBuilderProgram(kind: BuilderProgramKind, { newProgram, host, oldProgram, configFileParsingDiagnostics }: BuilderCreationParameters) {
        // Return same program if underlying program doesn't change
                ...
                if (isDeclarationFileName(fileName)) {
                    if (!outFile(state.compilerOptions)) {
                        ...
                        if (state.compilerOptions.composite) {
                            ...
                            // Don't write dts files if they didn't change
        ...
        function getSemanticDiagnostics(sourceFile?: SourceFile, cancellationToken?: CancellationToken): readonly Diagnostic[] {
            ...
            if (outFile(compilerOptions)) {
                Debug.assert(!state.semanticDiagnosticsPerFile);
                // We don't need to cache the diagnostics just return them from program
